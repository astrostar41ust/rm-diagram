...
public interface NoteRepository extends JpaRepository<Note, Long> {
    ...
    @Query("SELECT n FROM Note n WHERE n.userId = :userId AND n.deleted = false AND (LOWER(n.title) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(n.content) LIKE LOWER(CONCAT('%', :keyword, '%')))")
